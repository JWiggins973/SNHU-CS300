...
void selectionSort(vector<Bid>& bids) {
    ...
    for (pos = 0; pos < bidSize - 1; pos++) {
        // set min = pos
        min = pos;
        
        // loop over remaining elements to the right of position
        for (size_t j = pos + 1; j < bidSize; j++) {
        ...
        // if min != pos then min is smaller so swap
...
// Check if bids loaded if not load bids
void checkBidsLoaded(vector<Bid>& bids, string csvPath) {
    // check if bids loaded
    if (bids.empty()) {
        cout << "Bids not loaded." << endl;
...
int main(int argc, char* argv[]) {
    ...
    int choice = 0;
    while (choice != 9) {
        cout << "Menu (choose 1 first to load bids):" << endl;
        ...
        case 4:
            ...
            // Invoke the selection sort and report timing results (bid size -1 so no out of bound errors)
